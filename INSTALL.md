...
锁持有进程已退出时，下一次更新或卸载会立即回收锁；尚未写完的锁信息有 5 秒初始化
宽限期，仍在运行或无法确认已退出的进程不会仅因锁存在时间长而被驱逐。Windows 使用
只读的进程句柄状态检查。重复副本清理会保留指向目标目录本身的父目录别名，仅移除
独立的副本或链接。如果新版本已经安装成功、但旧目录清理失败，脚本返回非零并报告
残留路径，同时保留完整的新版本；不要将已经部分删除的旧目录用于回滚。
